...
export function LeaderReportsPage() {
  ...
  const fetchCommits = useCallback(async () => {
    ...
    try {
      const params = {
        ...(selectedMember !== 'all' ? { member: selectedMember } : {}),
        ...(selectedStatus !== 'all' ? { status: selectedStatus } : {}),
      };
      const res = await getCommitsApi(groupId, params);
  ...
  }, [groupId, selectedMember, selectedStatus]);
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {reports.map((r) => (
                  <option key={r.user?.user_id || r.report_id} value={r.user?.user_id || ''}>
              ...
              {commits.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="py-8 text-center text-muted-foreground">
                    {commitsLoading
                      ? 'Đang tải...'
                      : 'Chưa có commits. Hãy cấu hình GitHub và Sync.'}
                  </TableCell>
                </TableRow>
              ) : (
                commits.map((c, idx) => (
                  <TableRow key={c.commit_id || idx} className="hover:bg-muted/30">
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <AvatarCircle
                          name={c.author_name || c.author?.full_name || c.author_login || 'Unknown'}
                        />
                        <span className="text-sm font-medium">
                          {c.author_name || c.author?.full_name || c.author_login || '—'}
                        </span>
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      <Badge variant={c.match_status === 'unmatched' ? 'secondary' : 'outline'}>
